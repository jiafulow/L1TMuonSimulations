...
def drawer_book(histos, options):
    ...
    for ptbin in l1t_pt_vec:
        for modebin in l1t_mode_vec:
            ...
            htitle, nbinsx, xmin, xmax = "; #Delta (p_{T})/p_{T} {mode %s}; entries" % (get_l1t_mode_label(modebin)), 200, -8, 8
        ...
        for qualbin in l1t_quality_vec:
            ...
            htitle, nbinsx, xmin, xmax = "; #Delta (p_{T})/p_{T} {modes %s}; entries" % (get_l1t_quality_label(qualbin)), 200, -8, 8
            histos[hname] = TH1F(hname, htitle, nbinsx, xmin, xmax)
    ...
    for ptbin in l1t_pt_vec:
        for modebin in l1t_mode_vec:
            ...
            htitle, nbinsx, xmin, xmax, nbinsy, ymin, ymax = "; #eta; #Delta (p_{T})/p_{T} {mode %s}" % (get_l1t_mode_label(modebin)), 200, -8, 8, 26, 1.2, 2.5
            histos[hname] = TH2F(hname, htitle, nbinsy, ymin, ymax, nbinsx, xmin, xmax)
            histos[hname].indices = (modebin, ptbin)
        for qualbin in l1t_quality_vec:
            hname = "resolution_pt_vs_eta_in_qual%i_pt%i" % (qualbin, ptbin)
            htitle, nbinsx, xmin, xmax, nbinsy, ymin, ymax = "; #eta; #Delta (p_{T})/p_{T} {modes %s}" % (get_l1t_quality_label(qualbin)), 200, -8, 8, 26, 1.2, 2.5
            histos[hname] = TH2F(hname, htitle, nbinsy, ymin, ymax, nbinsx, xmin, xmax)
            histos[hname].indices = (qualbin, ptbin)
    ...
    for etabin in l1t_eta_vec:
        for modebin in l1t_mode_vec:
            ...
            htitle, nbinsx, xmin, xmax = "; p_{T} [GeV]; #Delta (p_{T})/p_{T} {mode %s}" % (get_l1t_mode_label(modebin)), 200, -8, 8
        ...
        for qualbin in l1t_quality_vec:
            ...
            htitle, nbinsx, xmin, xmax = "; p_{T} [GeV]; #Delta (p_{T})/p_{T} {modes %s}" % (get_l1t_quality_label(qualbin)), 200, -8, 8
            histos[hname] = TH2F(hname, htitle, len(myptbins)-1, np.array(myptbins), nbinsx, xmin, xmax)
