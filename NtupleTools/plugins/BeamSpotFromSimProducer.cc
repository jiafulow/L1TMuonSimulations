...
void BeamSpotFromSimProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if(!gotProcessParameterSet){
	...
	else if (type=="BetafuncEvtVtxGenerator") {

	  double betastar = ps.getParameterSet("VtxSmeared").getParameter<double>("BetaStar");
	  double emittance = ps.getParameterSet("VtxSmeared").getParameter<double>("Emittance");
	  sigmaX_ = sqrt(betastar * emittance / 2.0);
	  sigmaY_ = sigmaX_;
	  sigmaZ_ = ps.getParameterSet("VtxSmeared").getParameter<double>("SigmaZ");
      
	  meanX_ = ps.getParameterSet("VtxSmeared").getParameter<double>("X0");
	  meanY_ = ps.getParameterSet("VtxSmeared").getParameter<double>("Y0");
	  meanZ_ = ps.getParameterSet("VtxSmeared").getParameter<double>("Z0");
