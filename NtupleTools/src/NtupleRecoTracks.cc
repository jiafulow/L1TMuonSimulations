...
NtupleRecoTracks::NtupleRecoTracks(const edm::ParameterSet& iConfig) :
  ...
  maxN_    (iConfig.getParameter<unsigned>("maxN")) {
    ...
    token_ = consumes<reco::TrackCollection>(inputTag_);
    ...
    produces<std::vector<bool> >    (prefix_ + "highPurity"        + suffix_);
    produces<unsigned>              (prefix_ + "size"              + suffix_);
}

NtupleRecoTracks::~NtupleRecoTracks() {}

void NtupleRecoTracks::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    std::auto_ptr<std::vector<bool> >    v_highPurity       (new std::vector<bool>());
    ...
    if (!token_.isUninitialized())
        iEvent.getByToken(token_, tracks);

    if (tracks.isValid()) {
        edm::LogInfo("NtupleRecoTracks") << "Size: " << tracks->size();
        ...
        for (reco::TrackCollection::const_iterator it = tracks->begin(); it != tracks->end(); ++it) {
            ...
            v_highPurity       ->push_back(it->quality(reco::Track::TrackQuality::highPurity));
        ...
        *v_size = v_px->size();
        ...
        edm::LogError("NtupleRecoTracks") << "Cannot get the product: " << inputTag_;
    }
    ...
    iEvent.put(v_highPurity       , prefix_ + "highPurity"        + suffix_);
