...
NtupleRecoPFJets::NtupleRecoPFJets(const edm::ParameterSet& iConfig) :
  ...
  maxN_    (iConfig.getParameter<unsigned>("maxN")) {
    ...
    token_ = consumes<reco::PFJetCollection>(inputTag_);
    ...
    produces<std::vector<unsigned> > (prefix_ + "nConstituents"            + suffix_);
    produces<unsigned>               (prefix_ + "size"              + suffix_);
}

NtupleRecoPFJets::~NtupleRecoPFJets() {}

void NtupleRecoPFJets::produce(edm::Event& iEvent, const edm::EventSetup& iSetup) {
    ...
    std::auto_ptr<std::vector<unsigned> > v_nConstituents    (new std::vector<unsigned>());
    ...
    if (jets.isValid()) {
        edm::LogInfo("NtupleRecoPFJets") << "Size: " << jets->size();
        ...
        for (reco::PFJetCollection::const_iterator it = jets->begin(); it != jets->end(); ++it) {
            ...
            v_nConstituents    ->push_back(it->nConstituents());
        ...
        *v_size = v_px->size();
        ...
        edm::LogError("NtupleRecoPFJets") << "Cannot get the product: " << inputTag_;
    }
    ...
    iEvent.put(v_nConstituents    , prefix_ + "nConstituents"     + suffix_);
