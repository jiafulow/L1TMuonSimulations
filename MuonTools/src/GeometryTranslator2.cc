...
GeometryTranslator2::GeometryTranslator2():
  _geom_cache_id(0ULL) {
}
...
double
GeometryTranslator2::calculateGlobalEta(const TriggerPrimitive& tp) const {
  switch(tp.subsystem()) {
...
double
GeometryTranslator2::calculateGlobalPhi(const TriggerPrimitive& tp) const {
  switch(tp.subsystem()) {
  case TriggerPrimitive::kDT:
...
double
GeometryTranslator2::calculateBendAngle(const TriggerPrimitive& tp) const {
  switch(tp.subsystem()) {
  case TriggerPrimitive::kDT:
...
GlobalPoint
GeometryTranslator2::getGlobalPoint(const TriggerPrimitive& tp) const {
  switch(tp.subsystem()) {
  case TriggerPrimitive::kDT:
  ...
  case TriggerPrimitive::kRPC:
    ...
    break;
  default:
    const GlobalPoint ret(GlobalPoint::Polar(0.0, 0.0, std::nan("Invalid TP type!")));
    return ret;
    break;
  }
}

void GeometryTranslator2::checkAndUpdateGeometry(const edm::EventSetup& es) {
...
GlobalPoint
GeometryTranslator2::getRPCSpecificPoint(const TriggerPrimitive& tp) const {
...
double
GeometryTranslator2::calcRPCSpecificEta(const TriggerPrimitive& tp) const {
  return getRPCSpecificPoint(tp).eta();
}

double
GeometryTranslator2::calcRPCSpecificPhi(const TriggerPrimitive& tp) const {
...
double
GeometryTranslator2::calcRPCSpecificBend(const TriggerPrimitive& tp) const {
  return 0.0;
}
...
GeometryTranslator2::getCSCSpecificPoint(const TriggerPrimitive& tp) const {
  ...
  case 1:
    ...
    offset = 0.0 * CSCPatternLUT::get2007Position(pattern);  // FIXME
  }
...
double
GeometryTranslator2::calcCSCSpecificEta(const TriggerPrimitive& tp) const {
  return getCSCSpecificPoint(tp).eta();
}

double
GeometryTranslator2::calcCSCSpecificPhi(const TriggerPrimitive& tp) const {
  return getCSCSpecificPoint(tp).phi();
}

double
GeometryTranslator2::calcCSCSpecificBend(const TriggerPrimitive& tp) const {
  return 0.0;
}

GlobalPoint
GeometryTranslator2::calcDTSpecificPoint(const TriggerPrimitive& tp) const {
...
double
GeometryTranslator2::calcDTSpecificEta(const TriggerPrimitive& tp) const {
  return calcDTSpecificPoint(tp).eta();
}

double
GeometryTranslator2::calcDTSpecificPhi(const TriggerPrimitive& tp) const {
  return calcDTSpecificPoint(tp).phi();
}

// we have the bend except for station 3
double
GeometryTranslator2::calcDTSpecificBend(const TriggerPrimitive& tp) const {
  int bend = tp.getDTData().bendingAngle;
  double bendf = bend/512.0;
  return bendf;
}

bool GeometryTranslator2::
